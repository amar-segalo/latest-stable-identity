          ...
          <div className='row'>
            <div className='col-md-8 cart'>
                ...
                <div className='row'>
                  ...
                  {order.orderItems.length == 1 && (
                    <div className='col align-self-center text-right text-muted'>
                      {order.orderItems.length} proizvod
                    </div>
                  )}
                  {order.orderItems.length > 1 && (
                    <div className='col align-self-center text-right text-muted'>
                      {order.orderItems.length} proizvoda
                    </div>
                  )}
                </div>
              </div>
              {order.orderItems.map((orderItem, key) => {
                ...
                return (
                  <div className='row border-top mt-1' key={orderItem.id}>
            ...
            <div className='col-md-4 summary'>
              <div>
                <h5>
                  <b>Informacije o dostavi</b>
                </h5>
                <hr />

                <p>
                  Besplantna dostava:{' '}
                  <b style={{float: 'right'}}> {order.isFreeShipping ? 'Da' : 'Ne'}</b>
                </p>
                ...
                <p>
                  ...
                  <b style={{float: 'right'}}> {order.isPickupInStore ? 'Da' : 'Ne'} </b>
                </p>
              </div>

              <div className='mt-10'>
                <h5>
                  <b>Informacije o plaćanju</b>
                </h5>
                <hr />
                <p>
                  Način plaćanja :{' '}
                  <b style={{float: 'right'}}>
                    {order.paymentType.paymentTypeTranslations.find((x) => x.languageId == 1)?.name}{' '}
                  </b>
                </p>
                <p>
                  Ukupno za platiti :{' '}
              ...
              <div className='mt-10'>
                <h5>
                  <b>Loyalty informacije</b>
                </h5>
                <hr />

                <p>
                  Plaćeno bodovima :{' '}
                  <b style={{float: 'right'}}> {order.pointsLogs[0].points > 0 ? 'Ne' : 'Da'} </b>
                </p>
                {order.pointsLogs[0].points > 0 && (
                  <p>
                    Dobijeni bodovi za narudžbu:{' '}
                    <b style={{float: 'right'}}>{order.pointsLogs[0].points} </b>{' '}
                  </p>
                )}
                {order.pointsLogs[0].points < 0 && (
                  <p>
                    Potrošeni bodovi za narudžbu:{' '}
